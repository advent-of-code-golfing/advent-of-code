...
class MazeMap: 
    def __init__(self, maze_map: np.array): 
        ...
        self.known_points = [[Point(self.starting_loc, 
                                   STARTING_DIRECTION, 
                                   0, 
                                   0, 
                                   None)]]
    ...
    def find_checkpoints(self): 
        ...
        for x_coord, y_coord in zip(all_points[0], all_points[1]): 
            possible_surrounding_directions = self._check_surrounding_directions((x_coord, y_coord), all_directions)
            ...
            if len(possible_surrounding_directions) == 2:  
                if np.sum(np.abs(possible_surrounding_directions[0] + possible_surrounding_directions[1])) != 0: 
                    check_points.append((x_coord, y_coord))
    ...
    def _potential_connection(self, known_point: Point, coord: tuple) -> tuple: 
        ...
        assert((dist_vector[0]/abs_sum, dist_vector[1]/abs_sum) in ROTATION_MAP.keys())
    ...
    def check_connected_points(self, coord: tuple, compare_vs_all: bool = False, check_duplicates: bool = False):
        ...
        for known_point in checks: 
            connected, dist, direction, pathway = self._potential_connection(known_point, coord)   
            if connected: 
                new_known_point = Point(coord = coord,
                                        direction = direction,  
                                        score = dist, 
    ...
    def calc_final_score(self, part2: bool=False): 
        ...
        if not part2: 
            ...
            shortest_path_points = [p for p in self.known_points_flattened() if p.cum_score == shortest_path_length and p.coord == self.ending_loc]
