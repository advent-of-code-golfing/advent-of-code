...
def _get_perimeter_and_area_from_coords(region: list, farm_map_original: np.array, crop: int): 
    area = len(region)

    perimeter = 0
    for point in region: 
        neighbors = _neighboring_coords(point[0], point[1], len(farm_map_original), len(farm_map_original))
        ...
        while np.sum(farm_map == crop) != 0: 
            ...
            while len(boundaries) > 0: 
                ...
                for boundary in boundaries: 
                    temp = _neighboring_coords(boundary[0], boundary[1], len_rows, len_cols)
...
def q1(farm_map: np.array, mapping: dict) -> int:
    ...
    for crop in regions: 
        for region in regions[crop]: 
            ...
            area, perimeter = _get_perimeter_and_area_from_coords(region, farm_map, crop,)
...
def _count_right_angles(region, farm_map, crop): 
    right_angles = []
    for point in region: 
        c_1, c_2, c_3, c_4, c_5, c_6, c_7, c_8 = _extended_neighboring_coords(point[0], point[1])
        """
