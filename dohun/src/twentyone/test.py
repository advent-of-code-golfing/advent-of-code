...
def test_evaluation_and_inversion() -> None:
    ...
    assert(len('v<<A>>^A<A>AvA<^AA>A<vAAA>^A') == len(robot_2.invert_commands('<A^A>^^AvvvA')))
    assert(robot_2.test_inversion('<A^A>^^AvvvA'))

    robot_3 = Robot(KEYPAD_2, KEYPAD2_LOC)
    assert('v<<A>>^A<A>AvA<^AA>A<vAAA>^A' == robot_3.evaluate_commands('<vA<AA>>^AvAA<^A>A<v<A>>^AvA^A<vA>^A<v<A>^A>AAvA^A<v<A>A>^AAAvA<^A>A'))
    assert(len('<vA<AA>>^AvAA<^A>A<v<A>>^AvA^A<vA>^A<v<A>^A>AAvA^A<v<A>A>^AAAvA<^A>A') == len(robot_2.invert_commands('v<<A>>^A<A>AvA<^AA>A<vAAA>^A')))
