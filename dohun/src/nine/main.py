...
def q1(disk_representation: list) -> int:
    ...
    last_num = next(len_disk - j - 1 for j in range(len_disk) if disk_representation[len_disk - j - 1] != '.')

    while last_num > first_dot: 
        disk_representation[first_dot], disk_representation[last_num] = disk_representation[last_num], disk_representation[first_dot]
...
def q2(disk_representation: list, num_representation: list, evens: list, odds: list) -> int: 
    ...
    for i in range(len(evens)): 
        ...
        for current_space_num, current_space_available in enumerate(odds): 
            current_space_available = odds[current_space_num] - space_taken[current_space_num]
            if current_space_available == 0: 
                continue
            current_first_space_loc = sum(num_representation[:current_space_num*2 + 1]) + space_taken[current_space_num]
            if current_first_space_loc > current_first_file_loc: 
                break 

            if current_file_len < current_space_available: 
                disk_representation[current_first_space_loc: current_first_space_loc+current_file_len] = [current_index_num for _ in range(current_file_len)]
                disk_representation[current_first_file_loc: current_first_file_loc+current_file_len] = ['.' for _ in range(current_file_len)]
