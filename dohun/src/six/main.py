...
def _guard_one_iteration(map: np.array, 
    ...
    if not hurdle_exists: 
        ...
        if with_new_route_tracking: 
            return map, ROTATION_MAP[direction], len(hurdle), guard_tracker_map, new_route
    ...
    if direction == 'N': 
        ...
        if first_hurdle + 1 == len(hurdle): 
            map[loc_i, loc_j] = 2
            if with_new_route_tracking: 
                return map, ROTATION_MAP[direction], 0, guard_tracker_map, np.zeros_like(guard_tracker_map)
...
def _check_if_loop_exists(previous_stops: dict, direction: str, new_route_map: np.array, map: np.array, barriers: list) -> list: 
    ...
    if direction == 'W': 
        ...
        potential_locations = [x for x in potential_col_locations if x[0] >= row_min-1 and x[1] < row_max]
    ...
    if direction == 'E': 
        ...
        potential_col_locations = [x for x in potential_stops if x[1] > col_max]
        potential_locations = [x for x in potential_col_locations if x[0] > row_min and x[1] <= row_max+1]
    ...
    if direction == 'N': 
        ...
        potential_locations = [x for x in potential_row_locations if x[1] <= col_max + 1 and x[1] > col_min]
    ...
    if direction == 'S': 
        ...
        potential_row_locations = [x for x in potential_stops if x[0] > row_max]
        potential_locations = [x for x in potential_row_locations if x[1] < col_max and x[1] >= col_min - 1]
...
def q2(map: np.array, direction: str) -> int: 
    ...
    while guard_exists: 
        ...
        map, direction, dist_travelled, guard_tracker_map, new_route_map = _guard_one_iteration(map, 
                                                                                (loc_i, loc_j),
                                                                                direction, 
                                                                                guard_tracker_map, 
                                                                                with_new_route_tracking=True,)

        loop_loc = _check_if_loop_exists(previous_stops, direction, new_route_map, map, barriers)
...
def brute_force_find_loops(old_map: np.array, new_route_map: np.array, old_direction: str, road_num: int = 0): 
...
def q2_brute_force(map: np.array, direction: str): 
    ...
    while guard_exists: 
        ...
        new_loop_locs = brute_force_find_loops(old_map, new_route_map, old_direction, road_num)
