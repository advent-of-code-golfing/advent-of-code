    ...
    for i in range(num_rows): 
        for j in range(num_cols):
            ...
            if i <= num_rows - 4 and j <= num_cols - 4:
                search_array.append(''.join([str_list[i, j] for i, j in zip(range(i, i+4), range(j, j+4))])) 

            if i <= num_rows - 4 and j >= 3: 
                search_array.append(str_list[i, j] + str_list[i+1, j-1] + str_list[i+2, j-2] + str_list[i+3, j-3])
