...
def extract_numbers_from_block(text_block: str) -> tuple:
    ...
    extracted_numbers = [int(num) for tuple_item in numbers for num in tuple_item if num]
    
    eq = np.array([[extracted_numbers[0], extracted_numbers[2]], 
...
def _price_if_exists(sol_vector: np.array, eq_vector: np.array, care_about_inrange: bool = True) -> int: 
